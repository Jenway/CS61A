...
def about(topic):
    ...
    def about_topic(words):
        words = split(remove_punctuation(lower(words)))
        for i in words:
            if lower(i) in topic:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0 or len(reference_words) == 0:
        ...
        count:int = 0
        for x in list(zip(typed_words,reference_words)):
            if x[0] == x[1]:
                count +=1
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    return 12.0 * float(len(typed)) / float(elapsed)
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    words = list(zip(word_list,[diff_function(typed_word,word,limit) for word in word_list]))
    ans = min(words,key = lambda w:w[1])
    if ans[1] > limit:
        return typed_word
    else:
        return ans[0]
...
def sphinx_swaps(start, goal, limit):
    ...
    return 1+sphinx_swaps(start[1:],goal[1:],limit-1) if start[0] != goal[0] else sphinx_swaps(start[1:],goal[1:],limit)
    # END PROBLEM 6
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    count: int =0
    for x in list(zip(sofar,prompt)):
        if x[0] == x[1]:
            count+=1
        else:
            break
...
def fastest_words(match):
    ...
    for i in word_indices:
        times = [time(match,p,i) for p in player_indices]
        ans[times.index(min(times))].append(word_at(match,i))
    return ans
    # END PROBLEM 10
