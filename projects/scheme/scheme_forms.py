...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        symbol, expr = signature, expressions.rest.first
...
def do_and_form(expressions, env):
    ...
    if expressions.rest == nil:
        ...
        if is_scheme_true(ret): return ret
        return False
    ret = scheme_eval(expressions.first, env)
    if is_scheme_true(ret): return do_and_form(expressions.rest, env)
    return False
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    if expressions.rest is nil:
        ret = scheme_eval(expressions.first,env, True)
        if is_scheme_false(ret):return False
        return ret
    ret = scheme_eval(expressions.first, env)
    if is_scheme_false(ret): return do_or_form(expressions.rest,env)
    return ret
    # END PROBLEM 12
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            "*** YOUR CODE HERE ***"
            expr = clause.rest
            return eval_all(expr, env) if not expr is nil else test
            # END PROBLEM 13
